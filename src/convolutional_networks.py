...
class Conv(object):
            ...
            for f in range(F):
                for i in range(H_out):
                    for j in range(W_out):
                        ...
                        convolution_sum = torch.sum(patch * w[f])
                        
                        out[n, f, i, j] = convolution_sum + b[f]
    ...
    def backward(dout, cache):
        ...
        db = torch.sum(dout, dim=(0, 2, 3)) # For biases
            ...
            for f in range(F):
                for i in range(H_out):
                    for j in range(W_out):
                        ...
                        dx_padded[n, :, h_start:h_start+HH, w_start:w_start+WW] += d_out_ij * w[f]
            ...
            for c in range(C):
                for i in range(H_out):
                    for j in range(W_out):
                        ...
                        patch = x[n, c, h_start:h_start+pool_height, w_start:w_start+pool_width]
                        d_out_ij = dout[n, c, i, j]
                        
                        max_val = torch.max(patch)
                        mask = (patch == max_val)
                        
                        dx[n, c, h_start:h_start+pool_height, w_start:w_start+pool_width] += mask * d_out_ij
...
class ThreeLayerConvNet(object):
    ...
    def __init__(self,
        ...
        pool_out_h = H // 2
        pool_out_w = W // 2
        linear_input_dim = num_filters * pool_out_h * pool_out_w
        
        self.params['W2'] = torch.randn(linear_input_dim, hidden_dim, device=device, dtype=dtype) * weight_scale
  ...
  def __init__(self, input_dims=(3, 32, 32),
    ...
    for i, out_channels in enumerate(num_filters):
        ...
        if weight_scale == 'kaiming':
            self.params[f'W{layer_num}'] = kaiming_initializer(
                in_channels, 
                out_channels, 
                K=3, 
                relu=True, 
            ...
            self.params[f'W{layer_num}'] = torch.randn(out_channels, in_channels, 3, 3, device=device, dtype=dtype) * weight_scale
        
        self.params[f'b{layer_num}'] = torch.zeros(out_channels, device=device, dtype=dtype)
        
        if self.batchnorm:
            self.params[f'gamma{layer_num}'] = torch.ones(out_channels, device=device, dtype=dtype)
            self.params[f'beta{layer_num}'] = torch.zeros(out_channels, device=device, dtype=dtype)
        
        in_channels = out_channels

    final_h = H // (2**len(max_pools))
    final_w = W // (2**len(max_pools))
    linear_input_dim = num_filters[-1] * final_h * final_w
    
    last_layer_num = self.num_layers
    
    if weight_scale == 'kaiming':
        self.params[f'W{last_layer_num}'] = kaiming_initializer(
            linear_input_dim, 
            num_classes, 
            K=None, 
            relu=False, 
        ...
        self.params[f'W{last_layer_num}'] = torch.randn(linear_input_dim, num_classes, device=device, dtype=dtype) * weight_scale
        
    self.params[f'b{last_layer_num}'] = torch.zeros(num_classes, device=device, dtype=dtype)
  ...
  def loss(self, X, y=None):
    ...
    for i in range(self.num_layers - 1):
        ...
        if self.batchnorm:
            ...
            if use_pool:
                out, caches[layer_num] = Conv_BatchNorm_ReLU_Pool.forward(out, w, b, gamma, beta, conv_param, bn_param, pool_param)
            else:
                out, caches[layer_num] = Conv_BatchNorm_ReLU.forward(out, w, b, gamma, beta, conv_param, bn_param)
        else:
            if use_pool:
                out, caches[layer_num] = Conv_ReLU_Pool.forward(out, w, b, conv_param, pool_param)
            else:
                out, caches[layer_num] = Conv_ReLU.forward(out, w, b, conv_param)
    
    last_layer_num = self.num_layers
    w_last = self.params[f'W{last_layer_num}']
    b_last = self.params[f'b{last_layer_num}']
    scores, caches[last_layer_num] = Linear.forward(out.contiguous(), w_last, b_last)
    ...
    last_layer_num = self.num_layers
    dout, dw, db = Linear.backward(dscores, caches[last_layer_num])
    grads[f'W{last_layer_num}'] = dw + 2 * self.reg * self.params[f'W{last_layer_num}']
    grads[f'b{last_layer_num}'] = db

    for i in range(self.num_layers - 2, -1, -1):
        layer_num = i + 1
        cache = caches[layer_num]
        use_pool = i in self.max_pools
        ...
        grads[f'W{layer_num}'] = dw + 2 * self.reg * self.params[f'W{layer_num}']
...
def find_overfit_parameters():
    # weight_scale = 2e-4   # Experiment with this!
    # learning_rate = 1e-5  # Experiment with this!

    
    weight_scale = 0.2   # Experiment with this!
    learning_rate = 0.02  # Experiment with this!
...
class BatchNorm(object):
  ...
  def backward(dout, cache):
    ...
    ivar_sqrt = torch.sqrt(sample_var + eps)
    dsample_var = torch.sum(dx_normalized * (x - sample_mean) * (-0.5) * (ivar_sqrt**-3), dim=0)
    
    dsample_mean = torch.sum(dx_normalized * (-1 / ivar_sqrt), dim=0) + \
                   dsample_var * torch.mean(-2 * (x - sample_mean), dim=0)

    dx = (dx_normalized / ivar_sqrt) + \
  ...
  def backward_alt(dout, cache):
    ...
    dx_normalized = dout * gamma
    ivar_sqrt = 1.0 / torch.sqrt(sample_var + eps)
    
    dx = (1.0 / N) * ivar_sqrt * (
        N * dx_normalized 
        - torch.sum(dx_normalized, dim=0) 
        - x_normalized * torch.sum(dx_normalized * x_normalized, dim=0)
    )
...
class SpatialBatchNorm(object):
  ...
  def forward(x, gamma, beta, bn_param):
    ...
    N, C, H, W = x.shape
    x_reshaped = x.permute(0, 2, 3, 1).reshape(N * H * W, C)
